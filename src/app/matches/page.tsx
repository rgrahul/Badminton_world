...
function MatchAvatar({ name, src, color }: { name: string; src?: string | null; color: "green" | "blue" }) {
  ...
  return (
    <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center text-sm font-black text-black shadow-md flex-shrink-0 overflow-hidden`}>
      {src && !error ? (
...
export default function MatchesPage() {
  ...
  const getStatusBadge = (status: string) => {
    const configs = {
      NOT_STARTED: { bg: "bg-gray-500/20 text-gray-300", label: "Not Started" },
      IN_PROGRESS: { bg: "bg-cyan-500/20 text-cyan-300", label: "In Progress" },
      COMPLETED: { bg: "bg-emerald-500/20 text-emerald-300", label: "Completed" },
    }
    const config = configs[status as keyof typeof configs]
    return (
      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${config.bg}`}>
        {config.label}
      </span>
    )
  }
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-2">Manage and score your badminton matches</p>
          </div>
          {canManage && (
            <div className="flex gap-2 w-full sm:w-auto">
              <Link href="/matches/new" className="w-full sm:w-auto">
                <Button size="lg" className="w-full sm:w-auto bg-gradient-to-r from-emerald-500 to-cyan-500 text-black font-semibold hover:from-emerald-400 hover:to-cyan-400">
        ...
        {loading ? (
          ...
          <Card className="border-white/10 bg-white/[0.03]">
            <CardContent className="py-12 text-center">
              ...
              <Link href="/matches/new">
                <Button className="bg-gradient-to-r from-emerald-500 to-cyan-500 text-black font-semibold">Create Your First Match</Button>
              </Link>
            </CardContent>
          </Card>
        ) : viewMode === "grid" ? (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {matches.map((match) => (
              <Card key={match.id} className="hover:border-white/20 transition-all border-white/10 bg-white/[0.03]">
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="bg-emerald-500/10 p-3 rounded-lg border border-emerald-500/20">
                        ...
                        {match.sideAPlayer2 && (
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="font-semibold text-sm text-white truncate"><PlayerLink name={match.sideAPlayer2} playerMap={playerMap} /></div>
                            </div>
                          </div>
                        )}
                      </div>
                      <div className="bg-cyan-500/10 p-3 rounded-lg border border-cyan-500/20">
                        ...
                        {match.sideBPlayer2 && (
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="font-semibold text-sm text-white truncate"><PlayerLink name={match.sideBPlayer2} playerMap={playerMap} /></div>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex flex-col gap-2">
                      <Link href={`/matches/${match.id}`} className="w-full">
                        <Button variant="ghost" className="w-full border border-white/10 hover:bg-white/5 text-gray-300" size="sm">
                          View Details
                        </Button>
                      </Link>
                      {canManage && match.status === "IN_PROGRESS" && (
                        <Link href={`/matches/${match.id}/score`} className="w-full">
                          <Button className="w-full bg-cyan-500/20 text-cyan-300 hover:bg-cyan-500/30 border border-cyan-500/20" size="sm">
                            Continue Scoring
                          </Button>
                        </Link>
                      )}
                      {canManage && match.status === "NOT_STARTED" && (
                        <Link href={`/matches/${match.id}`} className="w-full">
                          <Button className="w-full bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 border border-emerald-500/20" size="sm">
                            Start Match
                          </Button>
                        </Link>
                      )}
                ...
                <CardContent className="p-4">
                  ...
                  <div className="block lg:hidden space-y-3">
                    <div>
                      ...
                      <div className="text-sm text-gray-400">{match.type}</div>
                    </div>

                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-emerald-500/10 p-2 rounded-lg border border-emerald-500/20">
                      ...
                      {match.status !== "NOT_STARTED" && (
                        <div className="flex items-center gap-1 bg-white/5 px-2 py-2 rounded-lg border border-white/10">
                          <div className="text-lg font-black tabular-nums text-emerald-400">{match.setsWonBySideA}</div>
                          <div className="text-gray-500 font-bold text-xs">-</div>
                          <div className="text-lg font-black tabular-nums text-cyan-400">{match.setsWonBySideB}</div>
                        </div>
                      )}

                      <div className="flex-1 bg-cyan-500/10 p-2 rounded-lg border border-cyan-500/20">
                        <div className="flex items-center gap-2 mb-1">
                          <MatchAvatar name={match.sideBPlayer1} src={match.sideBPlayer1Photo} color="blue" />
                          <div className="flex-1 min-w-0">
                            <div className="font-semibold text-sm text-white truncate flex items-center gap-1">
                    ...
                    <div className="flex gap-2">
                      <Link href={`/matches/${match.id}`} className="flex-1">
                        <Button variant="ghost" size="sm" className="w-full border border-white/10 hover:bg-white/5 text-gray-300 text-xs">
                          View
                        </Button>
                      </Link>
                      {match.status === "IN_PROGRESS" && (
                        <Link href={`/matches/${match.id}/score`} className="flex-1">
                          <Button size="sm" className="w-full bg-cyan-500/20 text-cyan-300 hover:bg-cyan-500/30 border border-cyan-500/20 text-xs">
                            Score
                          </Button>
                        </Link>
                      )}
                      {match.status === "NOT_STARTED" && (
                        <Link href={`/matches/${match.id}`} className="flex-1">
                          <Button className="w-full bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 border border-emerald-500/20 text-xs" size="sm">
                            Start
                          </Button>
                        </Link>
                      )}
                  ...
                  <div className="hidden lg:grid grid-cols-10 gap-4 items-center">
                    ...
                    <div className="col-span-4 flex items-center gap-3">
                      ...
                      {match.status !== "NOT_STARTED" && (
                        <div className="flex items-center gap-2 bg-white/5 px-3 py-2 rounded-lg border border-white/10">
                          <div className="text-xl font-black tabular-nums text-emerald-400">{match.setsWonBySideA}</div>
                          <div className="text-gray-500 font-bold text-sm">-</div>
                          <div className="text-xl font-black tabular-nums text-cyan-400">{match.setsWonBySideB}</div>
                        </div>
                      )}
                    ...
                    <div className="col-span-1 flex flex-col gap-2">
                      <Link href={`/matches/${match.id}`}>
                        <Button variant="ghost" size="sm" className="w-full border border-white/10 hover:bg-white/5 text-gray-300 text-xs whitespace-nowrap">
                          View
                        </Button>
                      </Link>
                      {match.status === "IN_PROGRESS" && (
                        <Link href={`/matches/${match.id}/score`}>
                          <Button size="sm" className="w-full bg-cyan-500/20 text-cyan-300 hover:bg-cyan-500/30 border border-cyan-500/20 text-xs whitespace-nowrap">
                            Score
                          </Button>
                        </Link>
                      )}
                      {match.status === "NOT_STARTED" && (
                        <Link href={`/matches/${match.id}`}>
                          <Button className="w-full bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 border border-emerald-500/20 text-xs whitespace-nowrap" size="sm">
                            Start
                          </Button>
                        </Link>
                      )}
